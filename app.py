...
import sys
import time
# saw this on stack overflow, giving it a try.
...
# if PLOT_DEBUG_LINES == 1, show lines from this many locators
MAX_NUM_LOCATOR_LINES = 5

DEFAULT_CONFIG = os.path.join(os.path.dirname(
    __file__), "../bt_host_positioning/config/positioning_config.json")
...
class TextGLViewWidget(gl.GLViewWidget):
    def __init__(self):
        ...
        # Key shall be the text to be displayed and value shall be [x, y, z]
        self.textPositions = {}
        if "renderText" not in dir(self):
            print(
                "WARNING! Text rendering is not supported in 3D mode. Please try to use pyqtgraph 0.11.0")
    ...
    def __init__(self):
        ...
        if PLOT_VIEW_3D:
            ...
            p1 = gl.GLSurfacePlotItem(
                z=z, shader='shaded', color=planeColor, glOptions='additive')
...
#      self.w.addItem(ygrid)
            ...
            # Try to load locator images from possible directories. If not found,
            # disable plotting locator images but continue running.
            self.plot_debug_locators = False
            if PLOT_DEBUG_LOCATORS:
                base = os.path.dirname(__file__)
                candidates = [
                    os.path.join(base, "img", "locator.png"),
                    os.path.join(base, "imgs", "locator.png"),
                ]
                candidates_back = [
                    os.path.join(base, "img", "locator_back.png"),
                    os.path.join(base, "imgs", "locator_back.png"),
                ]
                front_path = None
                back_path = None
                for p in candidates:
                ...
                if front_path and back_path:
                    try:
                        ...
                        imB = Image.open(back_path)
                        self.img_tex_back = np.array(imB)
                        self.img_scale = 4000.0
                        self.plot_debug_locators = True
                    except Exception:
                        print(
                            "Warning: failed to load locator images; continuing without them")
                else:
                    print(
                        "Locator images not found in 'img/' or 'imgs/' - skipping locator image rendering")
    ...
    def plot_room(self):
        ...
        if PLOT_VIEW_3D:
            ...
            self.m1 = gl.GLMeshItem(
                meshdata=md, smooth=True, color=shelfColor, glOptions='translucent')
            self.m1.translate(1.25, -5.9, 0.0)
            self.m1.scale(3.20, 0.57, 1.26)
            self.w.addItem(self.m1)

            # table
            self.m16 = gl.GLMeshItem(
                meshdata=md, smooth=True, color=tableColor, glOptions='translucent')
            self.m16.translate(1.27, -4.5, 0.7)
            self.m16.scale(2.0, 0.8, 0.05)
            self.w.addItem(self.m16)

    def plot_locator(self, loc_id):
        if (PLOT_VIEW_3D != 1) or (not getattr(self, 'plot_debug_locators', False)):
            return

        # Draw front image
        front = gl.GLImageItem(self.img_tex_front, glOptions='translucent')
        front.translate(-self.img_tex_front.shape[0] / (
            2.0 * self.img_scale), -self.img_tex_front.shape[1] / (2.0 * self.img_scale), 0.005)
        front.scale(1.0 / self.img_scale, 1.0 /
                    self.img_scale, 1.0 / self.img_scale)
        front.rotate(self.locators[loc_id]["orientation"]
                     ["z"], x=0, y=0, z=1, local=True)
        front.rotate(self.locators[loc_id]["orientation"]
                     ["y"], x=0, y=1, z=0, local=True)
        front.rotate(self.locators[loc_id]["orientation"]
                     ["x"], x=1, y=0, z=0, local=True)
        front.translate(self.img_tex_front.shape[0] / (
            2.0 * self.img_scale), self.img_tex_front.shape[1] / (2.0 * self.img_scale), -0.005)
        front.translate(self.locators[loc_id]["coordinate"]["x"], self.locators[loc_id]
                        ["coordinate"]["y"], self.locators[loc_id]["coordinate"]["z"])
        front.translate(-self.img_tex_front.shape[0] / (
            2.0), -self.img_tex_front.shape[1] / (2.0), 0.005 * self.img_scale, local=True)
        self.w.addItem(front)
        self.locators[loc_id]["front"] = front

        # Draw back image
        back = gl.GLImageItem(self.img_tex_back, glOptions='translucent')
        back.translate(-self.img_tex_back.shape[0] / (
            2.0 * self.img_scale), -self.img_tex_back.shape[1] / (2.0 * self.img_scale), -0.005)
        back.scale(1.0 / self.img_scale, 1.0 /
                   self.img_scale, 1.0 / self.img_scale)
        back.rotate(self.locators[loc_id]["orientation"]
                    ["z"], x=0, y=0, z=1, local=True)
        back.rotate(self.locators[loc_id]["orientation"]
                    ["y"], x=0, y=1, z=0, local=True)
        back.rotate(self.locators[loc_id]["orientation"]
                    ["x"], x=1, y=0, z=0, local=True)
        back.translate(self.img_tex_back.shape[0] / (2.0 * self.img_scale),
                       self.img_tex_back.shape[1] / (2.0 * self.img_scale), 0.005)
        back.translate(self.locators[loc_id]["coordinate"]["x"], self.locators[loc_id]
                       ["coordinate"]["y"], self.locators[loc_id]["coordinate"]["z"])
        back.translate(-self.img_tex_back.shape[0] / (
            2.0), -self.img_tex_back.shape[1] / (2.0), -0.005 * self.img_scale, local=True)
        self.w.addItem(back)
    ...
    def plot_line(self, tag_id, loc_id):
        ...
        if "line" not in self.tags[tag_id]["angle"][loc_id]:
            # Draw new line
            self.tags[tag_id]["angle"][loc_id]["line"] = gl.GLLinePlotItem(
                pos=pos, color=self.tags[tag_id]["color"])
    ...
    def plot_marker(self, tag_id):
        ...
        if "marker" not in self.tags[tag_id]:
            ...
            # Draw new marker
            if PLOT_VIEW_3D:
                self.tags[tag_id]["marker"] = gl.GLScatterPlotItem(
                    pos=pos, color=color, size=size, pxMode=pxMode)
                self.tags[tag_id]["marker"].setGLOptions('translucent')
                self.w.addItem(self.tags[tag_id]["marker"])

            else:
                # PLOT_VIEW_2D
                brushColor = QtGui.QColor(
                    color[0] * 255, color[1] * 255, color[2] * 255)

                self.tags[tag_id]["marker"] = pg.ScatterPlotItem(pen=pg.mkPen(
                    width=1), brush=brushColor, symbol='o', size=size, pxMode=pxMode)
        ...
        if PLOT_VIEW_3D:
            if self.marker_trace and self.numEstimatesPlotted % self.markerTraceStep == 0:
                trace = gl.GLScatterPlotItem(
                    pos=pos, color=[0.6, 0.3, 0.8, 1.0], size=10)
    ...
    def plot_text(self, tag_id):
        ...
        if PLOT_VIEW_3D:
            ...
            if "text" not in self.tags[tag_id]:
                # Draw new text
                self.tags[tag_id]["text"] = pg.TextItem(
                    tag_id, color=(200, 200, 200), fill=None)
    ...
    def parse_config(self, conf_file):
        ...
        if not os.path.exists(conf_file):
            ...
            print(
                "Provide a configuration file with -c or create the default at this path.")
            sys.exit(1)
        ...
        for loc in locators:
            ...
            m = re.match(
                r"^ble-(?P<address_type>[A-Za-z]+)-(?P<address>[0-9A-Fa-f]+)$", loc_id)
            if m is None:
                raise Exception("Invalid locator id format: {}".format(loc_id))
            loc_id = "ble-{}-{}".format(m.group("address_type").lower(),
                                        m.group("address").upper())
    ...
    def add_tag(self, tag_id):
        ...
        self.tags[tag_id]["color"] = [random.uniform(
            0, 1), random.uniform(0, 1), random.uniform(0, 1), 1.0]
    ...
    if len(arglist) > 1:
        try:
            ...
            raise argparse.ArgumentTypeError(
                "Invalid port number: " + arglist[1]) from val
    return retval
...
def on_message(client, userdata, msg):
    ...
    if m is not None:
        ...
        m = re.match(
            r"silabs/aoa/angle/(?P<loc_id>.+)/(?P<tag_id>.+)", msg.topic)
        if m is not None:
            entry = json.loads(msg.payload)
...
def main():
    ...
    parser.add_argument("-c", metavar="CONFIG_FILE",
                        help="Configuration file path", default=DEFAULT_CONFIG)
    parser.add_argument("-m", metavar="HOST[:PORT]", help="MQTT broker connection parameters",
                        default=DEFAULT_CONNECTION, type=mqtt_conn_type)
